...
//   );
// }
/* ==================== 1. STYLE SELECTION STEP ==================== */
export function StepSuitStyle({ data, onChange, product }: StepProps) {
  const stylesGroup = product?.customizationGroups?.find(
    (g: any) => g.name.toLowerCase().includes("style") || g.name.toLowerCase().includes("fit")
  );

  const fallbackStyles = [
    { name: "Slim Fit", desc: "Precision tapered cut closer to the body, emphasizing natural shoulders", delta: 0 },
    { name: "Classic Fit", desc: "Traditional clean drape providing supreme comfort and effortless mobility", delta: 0 },
    { name: "Double Breasted", desc: "Timeless 6-button front styling with elegant Peak Lapels (+£45.00)", delta: 45 },
    { name: "Unstructured Blazer", desc: "Relaxed soft-shouldered canvas perfect for trans-seasonal smart casual layers", delta: -20 }
  ];

  if (!stylesGroup) {
    return (
      <div className="space-y-6">
        <h3 className="text-sm font-semibold tracking-wider text-zinc-400 uppercase">Select Silhouette Drape</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {fallbackStyles.map((s) => (
            <button
              key={s.name}
              onClick={() => onChange({ style: s.name })}
              className={`p-6 rounded-lg border text-left transition-all ${
                data.style === s.name 
                  ? "border-[#c9a96e] bg-[#c9a96e]/5 text-[#f5f0eb]" 
                  : "border-white/10 bg-white/5 text-zinc-400 hover:border-white/20 hover:text-white"
              }`}
            >
              <div className="flex justify-between items-center mb-2">
                <span className="font-serif font-bold text-lg">{s.name}</span>
                {s.delta !== 0 && (
                  <span className="text-xs font-mono text-[#c9a96e]">
                    {s.delta > 0 ? `+£${s.delta.toFixed(2)}` : `-£${Math.abs(s.delta).toFixed(2)}`}
                  </span>
                )}
              </div>
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-sm font-semibold tracking-wider text-zinc-400 uppercase">{stylesGroup.name}</h3>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {stylesGroup.options?.map((opt: any) => (
          <button
            key={opt.id}
          ...
          >
            <div className="flex justify-between items-center mb-2">
              <span className="font-serif font-bold text-lg">{opt.value}</span>
              {Number(opt.price_delta || opt.priceDelta) > 0 && (
                <span className="text-xs font-mono text-[#c9a96e]">
                  +£{Number(opt.price_delta || opt.priceDelta).toFixed(2)}
                </span>
              )}
            </div>
            {opt.description && <p className="text-xs leading-relaxed text-zinc-500">{opt.description}</p>}
          </button>
        ))}
      </div>
    </div>
