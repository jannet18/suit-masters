import { boolean, integer, pgTable, serial, varchar, uuid } from "drizzle-orm/pg-core";
...
export const paymentMethodTable = pgTable("payment_method", {
    id: serial("id").primaryKey(),
    payment_type_id: integer("payment_type_id").notNull().references(() => paymentsTypeTable.id),
...
export const paymentMethodRelations = relations(paymentMethodTable, ({one}) => ({
    // user: one(usersTable, {
